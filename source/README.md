...
В папке `scripts/` рекомендуется хранить все вспомогательные скрипты для запуска обучения, инференса, конвертации, экспорта моделей и т.д. Это облегчает навигацию и отделяет основной код библиотеки от утилит и экспериментов.

Пример содержимого:
- `train.py`, `validate.py`, `inference.py` — основные скрипты для работы с моделями
- `onnx_export.py`, `onnx_validate.py` — экспорт и валидация ONNX
- `preprocess_image.py` — подготовка изображений
- `convert/` — скрипты для конвертации моделей из других фреймворков
- `benchmark.py`, `bulk_runner.py`, `avg_checkpoints.py` и др. — утилиты для экспериментов и анализа

**Рекомендуется запускать скрипты из этой папки, чтобы не смешивать их с основным кодом библиотеки.**

### input/ и output/

Папки `input/` и `output/` используются для хранения временных данных, тестовых изображений, промежуточных результатов и других файлов, необходимых для локального запуска и тестирования. Эти папки исключены из публикации и не должны содержать важные или уникальные данные.

**Рекомендуется очищать эти папки перед публикацией и не хранить в них чувствительную или важную информацию.**
...
Проект организован по принципу разделения основного кода, тестов, документации, скриптов, результатов экспериментов и весов моделей по отдельным папкам. Временные, большие и чувствительные файлы исключаются из публикации с помощью `.gitignore` и `MANIFEST.in`, но сохраняются локально для удобства работы и воспроизведения экспериментов.

- `timm/` — основной код библиотеки
- `tests/` — модульные тесты
- `scripts/` — все вспомогательные скрипты для запуска, обучения, инференса, конвертации, экспорта и анализа. Скрипты из корня рекомендуется переместить сюда для удобства.
- `models/` — веса моделей (исключены из публикации)
- `results/` — результаты экспериментов (исключены из публикации)
- `hfdocs/` — документация
- `requirements.txt`, `requirements-dev.txt`, `pyproject.toml`, `setup.cfg` — файлы установки и зависимостей
- `README.md`, `CONTRIBUTING.md`, `UPGRADING.md` — документация
- `.gitignore`, `MANIFEST.in` — файлы управления публикацией

### Корневые файлы

- `requirements.txt`, `requirements-dev.txt` — основные и дополнительные зависимости
- `pyproject.toml`, `setup.cfg` — конфигурация сборки и установки
- `MANIFEST.in` — управление включением/исключением файлов при публикации
- `README.md` — основная документация по проекту
- `CONTRIBUTING.md` — рекомендации по участию в разработке
- `UPGRADING.md` — инструкции по обновлению

### hfdocs/

Папка `hfdocs/` содержит документацию по использованию библиотеки, описания моделей, инструкции по установке и запуску, а также примеры. Рекомендуется поддерживать документацию в актуальном состоянии для удобства пользователей и разработчиков.

### models/

Папка `models/` предназначена для хранения весов обученных моделей и связанных с ними метаданных (например, args.yaml, summary.csv). Веса и результаты экспериментов исключены из публикации, но сохраняются локально для удобства воспроизведения и анализа.

**Рекомендуется не хранить большие файлы весов в публичных репозиториях, а использовать сторонние хранилища или релизы, если требуется распространение.**

### results/

Папка `results/` содержит результаты экспериментов, такие как метрики, логи, итоговые таблицы (CSV) и вспомогательные скрипты для анализа. Эти данные исключены из публикации, но сохраняются локально для удобства анализа и воспроизведения экспериментов.

**Рекомендуется хранить только необходимые для анализа результаты, а большие или промежуточные файлы — удалять или архивировать.**

### tests/

Папка `tests/` содержит модульные и интеграционные тесты для проверки корректности работы кода. Рекомендуется структурировать тесты по функциональным областям (например, test_layers.py, test_models.py) и поддерживать их актуальность при изменении кода.

### timm/

Папка `timm/` содержит основной код библиотеки: реализации моделей, слоёв, функций потерь, оптимизаторов, загрузчиков данных и утилит. Рекомендуется поддерживать чистую и логичную структуру внутри этой папки, разделяя код по функциональным подпапкам (`models/`, `layers/`, `data/`, `loss/`, `optim/`, `scheduler/`, `utils/`).

### .gitignore

Файл `.gitignore` содержит список файлов и папок, которые не должны попадать в систему контроля версий и публикацию (например, временные файлы, веса моделей, результаты экспериментов, виртуальные окружения и т.д.). Рекомендуется регулярно обновлять этот файл при добавлении новых временных или больших файлов.

### MANIFEST.in

Файл `MANIFEST.in` управляет тем, какие файлы и папки включаются или исключаются при публикации пакета (например, на PyPI). Важно исключать из публикации большие, временные и чувствительные данные, чтобы минимизировать размер пакета и повысить безопасность.
